...
**运行机制（v2.0.0：监听 ➔ 提取 ➔ 签到，全流程自动化闭环）**

本脚本采用事件监听驱动架构，**彻底废除定时轮询与手动参数输入**：

1. **监听（Listen）**：确保 Loon 开启且 MitM 证书受信任（包含 `*.app.coc.10086.cn, wx.10086.cn`）；打开中国移动 APP 即可触发流量监听；
2. **提取（Extract）**：脚本自动从客户端原生请求体中解密提取 **会话 Cookie、账号 UID 与真实 11 位手机号**（多账号各自以 UID 为索引独立绑定），全过程由脚本自动完成，**无需在任何地方手动配置或输入手机号**；
3. **触发签到（Trigger Sign）**：提取完成后，立即在同一次请求上下文内联执行当日签到、累签阶梯领奖与话费/流量/通话余量查询，合并为一条系统通知推送；
4. **防并发与节流**：执行锁（120 秒）防止 APP 启动并发请求导致重复签到；当日完成标记防止同一账号同日重复弹窗；多账号切换登录时各自独立触发；
5. **手动点测**：如需随时查看最新话费与流量卡片，可在 Loon 脚本列表中直接点击 **「中国移动手动签到」** 的【运行】，脚本直接复用沙盒凭据执行，零参数依赖。
