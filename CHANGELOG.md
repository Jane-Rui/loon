...
### Fixed
- **中国移动 (`Plugin/10086.js` v2.4.1 & `Plugin/10086_task.js` v2.4.1) 优化通知排版与根除重复资产行截断**：
  - **彻底清除正文重复资产行**：副标题已置顶单行加粗直显「💰 话费 ｜ 📶 流量 ｜ 📞 通话」核心三联排，正文彻底剔除重复的第一行资产文本，根治两行完全相同数据导致的视觉堆叠、信息遮挡与末尾文字截断（`26分钟...`）；
  - **前置直显签到打卡状态**：正文首行直接显示签到打卡结果及当月累计天数（如 `📅 今日已完成签到（本月累计 1 天）` 或 `🎉 签到成功（本月累计 2 天）`），确保 iOS 横幅与锁屏折叠卡片下行行关键、层级分明、零冗余信息；
  - **标题手机号兜底强绑定**：通知标题在资产接口未返回手机号时，严格使用当前账号 UID 绑定的手机号进行掩码展示，杜绝回退显示应用默认名。
