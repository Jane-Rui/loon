...
### Fixed
- **中国移动 (`Plugin/10086.js` v2.2.0 & `Plugin/10086_task.js` v2.2.0)** 彻底修复多重通知与排版重塑：
  - **消除双重通知**：无 UID 会话（如 H5 页面、统计埋点等）仅作为数据捕获源绝不触发签到；新增全局 60 秒原子硬锁（`cmcc_global_sign_lock`）并在首次判定时立即写盘，彻底杜绝并发请求导致的两次重复执行与弹窗；
  - **核心资产三联排置顶（直显免展开）**：将「💰 话费 ｜ 📶 流量 ｜ 📞 通话」核心资产数据直接移入 iOS 通知二级标题（Subtitle）并加粗置顶，默认折叠状态下第一眼直接可见，无需长按展开；
  - **通知文案极净化**：彻底剔除「🔑 会话捕获...」、「🎁 今日暂无待领取...」等技术噪音与空洞文案；标题直显「中国移动 · 手机号掩码」，状态精炼为「今日已签到 / 签到成功」，正文紧凑直观。
